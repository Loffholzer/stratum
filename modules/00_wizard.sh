...
wizard_run() {
    ...
    echo "------------------------------------------------------------------------------"
    read -p "Ziel-Datenträger (z.B. /dev/nvme0n1 oder /dev/sda): " WIZ_DISK
    if [[ ! -b "$WIZ_DISK" ]]; then
    ...
    read -p "Vollverschlüsselung (LUKS2) aktivieren? [j/N]: " WIZ_LUKS
    case "$WIZ_LUKS" in
        [jJ]*|[yY]*)
            export USE_LUKS="yes"
            while true; do
                read -s -p "LUKS Passwort festlegen: " WIZ_LUKS_PASS1; echo
                read -s -p "LUKS Passwort wiederholen: " WIZ_LUKS_PASS2; echo
    ...
    read -p "Hostname [Standard: stratum-pc]: " WIZ_HOST
    export HOSTNAME="${WIZ_HOST:-stratum-pc}"

    read -p "Hauptbenutzer [Standard: admin]: " WIZ_USER
    export USERNAME="${WIZ_USER:-admin}"

    while true; do
        read -s -p "Passwort für Benutzer '$USERNAME': " WIZ_USER_PASS1; echo
        read -s -p "Passwort wiederholen: " WIZ_USER_PASS2; echo
    ...
    read -p "Deployment jetzt starten? [j/N]: " WIZ_START
    case "$WIZ_START" in
